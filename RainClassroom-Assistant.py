...
class Ykt():
    ...
    def authorization(self):
        ...
        if js_user_info['data'][0]['user_id'] != def_user_id:
            messager = '对不起!\n姓名：'+js_user_info['data'][0]['name']+' 学号：'+str(js_user_info['data'][0]['school_number'])+' 授权ID：'+str(js_user_info['data'][0]['user_id'])+'\n 暂未获得授权，请联系QQ：1252811289 授权,谢谢！'
    ...
    def get_course_exams(self,classroom_id):
        ...
        """
        #message_list = []
        original_exam_list = self.session.get(url=self.exam_url + str(classroom_id) + '?actype=4&prev_id=-1&offset=99999&sort=-1',headers = self.headers2)
    ...
    def _process_cookie(self):
        ...
        with open('./cookie.txt','r') as f:  
            ...
            #print(info_list)
            csrf = re.search('^#LWP-Cookies-2.0.*?csrftoken=(.*?); path="/";.*?Set-Cookie3:.*?httponly=None; version=0',info_list,re.S)
            sessionid = re.search('^#LWP-Cookies-2.0.*?path="/".*?sessionid=(.*?); path="/";.*?httponly=None; version=0',info_list,re.S)
            cookie_dict = {
                'csrf':csrf.group(1),
